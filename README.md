...
The question is not whether an AI agent could infer all dependencies from code. Given unlimited compute and context, it likely could. The question is whether that inference is efficient or reliable in practice. Modern AI coding agents operate under strict context and compute constraints. Constitutional architecture compresses the semantic structure of the system into a compact knowledge layer that agents can consult before reading code. This dramatically reduces the search space of possible dependencies and prevents incorrect assumptions before they occur.
