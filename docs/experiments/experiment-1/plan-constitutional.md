# Plan: Grade Forgiveness (Drop Lowest Grade) — Constitutional Path

*Generated with full constitutional architecture (CLAUDE.md, charters, cross_cutting.md, risk-awareness reference note).*
...
**Critical nuance identified from charters**: `Grade.score` is already **penalty-adjusted** (tripwire from `infrastructure.md` L91 and `cross_cutting.md` "Late Penalty Timing"). This means the "lowest grade" correctly reflects late penalties — a student who scored 90 raw but got a 30% penalty has a `Grade.score` of 63. That 63 is the right value to compare. No special handling of late penalties is needed.
...
**No new code needed** — but this is the step the control group will likely miss.
...
This is the same pattern documented in `cross_cutting.md` "Rubric change amplification" — a course-level setting change that affects all existing grades.
...
Per the living docs convention:
- Update `models_courses.md` — add `grade_forgiveness` field to Course
- Update `infrastructure.md` — document the change to `calculate_course_grade()`
- Update `cross_cutting.md` — add grade forgiveness to "Cached Aggregates Pattern" (toggling the setting requires bulk recalculation)
- Update `docs/flows/grading-flow.md` — mention forgiveness in the grading public record
...
**Confidence**: High — the plan was validated against 4 charters. The tripwire about penalty-adjusted scores was surfaced and confirmed to be a non-issue (the adjusted score is the correct value to compare). The cascade chain is explicitly documented and the plan follows it.

**What could still surprise us**: If `calculate_course_grade` is called from anywhere besides `recalculate_grade_cache` — but the charter says it isn't.
